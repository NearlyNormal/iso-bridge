...
To add a new format pair (e.g., MT940 to camt.053):

1. Create `config/mappings/mt940-camt053.js` with a `MAPPING_MT940_CAMT053` variable containing the field-by-field mapping array.

2. Add a `<script>` tag in `index.html` to load it:
   ```html
   <script src="config/mappings/mt940-camt053.js"></script>
   ```
...
4. Add the corresponding transform functions in `app.js` if the new format requires parsing or building logic that doesn't already exist.

The engine discovers the new path automatically. Any country whose `formats` array includes both `MT940` and `camt.053` will show the path in its format selectors.
